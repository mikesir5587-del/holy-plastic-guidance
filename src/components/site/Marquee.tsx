          ...
          <li key={copy} aria-hidden={copy === 1} className="flex shrink-0">
            <ul className="flex shrink-0 items-center">
              {items.map((item) => (
                ...
                >
                  <span className="text-[1.35rem] text-white/60 transition-colors duration-300 group-hover:text-[color:var(--cyan)] sm:text-[1.7rem]">
                    {item.icon}
                  </span>
                  <span className="text-[0.72rem] font-semibold tracking-[0.22em] text-white/45 uppercase transition-colors duration-300 group-hover:text-white/85 sm:text-[0.8rem]">
                    {item.name}
                  </span>
                </li>
              ))}
...
export function Marquee({ rowA, rowB }: { rowA: MarqueeItem[]; rowB: MarqueeItem[] }) {
  return (
    <div className="marquee-hold" tabIndex={-1}>
      <Row items={rowA} />
